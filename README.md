...
全链路 E2E 使用仅限测试的 Fake LLM 和 hash embedding，不读取真实 API Key，也不会产生模型调用费用。它覆盖注册、登录、TXT 上传与解析、简历/JD 保存、匹配分析、简历优化、项目改写、任务状态流转、两轮会话及消息持久化。测试环境使用独立的 Compose project、端口和数据卷，结束后自动清理。
